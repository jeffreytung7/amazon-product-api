...
class AmazonScraper {
    ...
    httpRequest({ uri, method, qs, json, body, form }) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                // const response = await axios.get(uri ? `${this.mainHost}/${uri}` : this.mainHost, options);
    ...
    grabReviews(body) {
         ...
         */
        this.reviewMetadata.total_reviews = parseInt($('.averageStarRatingNumerical').text().replace(/,/gi, ''), 10);
        ...
        for (let key in scrapingResult) {
            try {
                ...
                if (ratingSearch) {
                    ...
                    let ratingValue = ratingSearch.parent.parent.parent.next.attribs['aria-label'];
                    if (!ratingValue) {
                        ratingValue = $(ratingSearch.parent.parent.parent.next).text();
                    }
                    scrapingResult[key].reviews.total_reviews = parseInt(ratingValue.replace(/\,/g, ''), 10);
